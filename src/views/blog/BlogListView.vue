...
import { loadAllPosts } from '@/utils/markdown/loader'
import type { PostEntry } from '@/utils/markdown/loader'
...
const filteredPosts = computed<PostEntry[]>(() => {
  if (!filterCategory.value) return allPosts
  return allPosts.filter((p) => p.category === filterCategory.value)
})
